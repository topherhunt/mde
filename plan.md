...
|-----------------|-------------------------------|--------------------------------------------------------------|
| Shell           | Electron (electron-forge)     | Mature, good testing story, familiar web tech                |
| UI framework    | React                         | First-class TipTap bindings; enormous ecosystem              |
| Editor          | TipTap (ProseMirror)          | Structured document model, Markdown serde, toolbar commands  |
| Markdown parse  | markdown-it (via TipTap)      | TipTap's Markdown extension uses this under the hood         |
| Markdown serial | tiptap-markdown               | Serialize TipTap document back to Markdown on save           |
| Syntax highlight| lowlight (via CodeBlockLowlight) | TipTap extension, highlight.js-based, zero config         |
| PDF export      | Electron printToPDF           | Free -- renders current editor content                       |
| DOCX export     | html-docx-js                  | "Good enough" .docx from HTML. Not high fidelity             |
| Find/replace    | ProseMirror search plugin     | Or custom floating bar using TipTap commands                 |
| File watching   | chokidar (or Node fs.watch)   | Detect on-disk changes to open files                         |
| Testing         | Playwright (Electron mode)    | Headless E2E tests, officially supported                     |
| Package manager | npm                           | Simple, no fuss                                              |
